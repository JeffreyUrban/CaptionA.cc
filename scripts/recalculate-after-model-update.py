...
recalculates crop region, and marks captions for review if crop region changed.
...
def calculate_crop_region_from_predictions(conn: sqlite3.Connection) -> dict | None:
    """Calculate crop region from predicted 'in' boxes.

    Returns new crop region dict or None if no predictions available.
    """
    cursor = conn.cursor()
    ...
    # Calculate bounding region of all 'in' boxes
...
def recalculate_video(db_path: Path, dry_run: bool = False) -> tuple[bool, str, dict | None]:
    """Recalculate crop region for a video if model version changed.
    ...
    try:
        ...
        # Calculate new crop region
        new_crop_region = calculate_crop_region_from_predictions(conn)

        if not new_crop_region:
        ...
        # Check if crop region changed
        crop_region_changed = (
            new_crop_region["crop_left"] != old_crop_region["crop_left"]
            or new_crop_region["crop_top"] != old_crop_region["crop_top"]
            or new_crop_region["crop_right"] != old_crop_region["crop_right"]
            or new_crop_region["crop_bottom"] != old_crop_region["crop_bottom"]
        )

        if not crop_region_changed:
            # Crop Region unchanged - just update version
            cursor.execute(
                "UPDATE video_layout_config SET analysis_model_version = ? WHERE id = 1",
                (new_version,),
            )
            ...
            return (
                True,
                "Recalculated, crop region unchanged",
                {
        ...
        # Crop region changed - update config and mark captions pending
        cursor.execute(
            """
        ...
        """,
            (
                new_crop_region["crop_left"],
                new_crop_region["crop_top"],
                new_crop_region["crop_right"],
                new_crop_region["crop_bottom"],
                new_version,
            ),
        )
        ...
        old_crop_region_str = (
            f"[{old_crop_region['crop_left']},{old_crop_region['crop_top']},"
            f"{old_crop_region['crop_right']},{old_crop_region['crop_bottom']}]"
        )
        new_crop_region_str = (
            f"[{new_crop_region['crop_left']},{new_crop_region['crop_top']},"
            f"{new_crop_region['crop_right']},{new_crop_region['crop_bottom']}]"
        )
        return (
            True,
            f"Crop Region changed: {old_crop_region_str} → {new_crop_region_str}, marked {captions_marked} captions pending",
            {
                "action": "recalculated_changed",
                "display_path": display_path,
                "old_version": old_version,
                "new_version": new_version,
                "old_crop_region": old_crop_region,
                "new_crop_region": new_crop_region,
...
def main():
    parser = argparse.ArgumentParser(description="Recalculate crop region after model updates (background job)")
    ...
    print(f"  ✅ Recalculated (crop region unchanged): {stats['recalculated_unchanged']}")
    print(f"  🎯 Recalculated (crop region changed): {stats['recalculated_changed']}")
    print(f"  ❌ Errors: {stats['errors']}")
    print("=" * 70)

    if changed_videos:
        print(f"\n📋 Videos with changed crop region ({len(changed_videos)}):\n")
        for video in changed_videos[:20]:
            print(f"  • {video['display_path']}")
            print(f"    Old: {video['old_crop_region']}")
            print(f"    New: {video['new_crop_region']}")
