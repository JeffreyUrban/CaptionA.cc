...
Get boundary inference running on Modal in 5 steps:
...
python scripts/upload_model_to_modal.py \
  --checkpoint ../../models/caption_boundaries/fusion_lora_spatial_mrn0fkfd.pt \
...
- **Function**: `run_boundary_inference_batch`
  - GPU: A10G (24GB VRAM)
  - Cost: ~$1.10/hour (only when running)
  - Timeout: 1 hour max
  - Concurrency: 5 parallel max

- **Volume**: `boundary-models`
...
```
videos/{tenant_id}/{video_id}/boundaries/v1_model-mrn0fkfd_run-{uuid}.db
```
