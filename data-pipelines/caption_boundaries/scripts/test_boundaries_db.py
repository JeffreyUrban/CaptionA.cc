"""Test script for boundaries database operations.
...
def test_create_and_read_db():
    """Test creating and reading boundaries database."""
    console.print("\n[cyan]Test 3: create_boundaries_db() and read_boundaries_db()[/cyan]")

    # Create temporary directory
    with tempfile.TemporaryDirectory() as tmpdir:
        db_path = Path(tmpdir) / "test_boundaries.db"
        ...
        created_db_path = create_boundaries_db(
            db_path=db_path,
            cropped_frames_version=1,
        ...
        # Read database back
        metadata, read_results = read_boundaries_db(db_path)
...
def test_large_batch():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        db_path = Path(tmpdir) / "large_boundaries.db"
...
def main():
    """Run all tests."""
    console.print("[bold cyan]Testing Boundaries Database Operations[/bold cyan]")
    console.print("=" * 60)
