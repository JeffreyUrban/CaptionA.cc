...
3. Labels pairs based on caption boundaries
4. Stores in TrainingSample table with full provenance

Labels:
- same: Both frames in same caption
- different: Frames in different captions (boundary transition)
...
def find_videos_with_confirmed_boundaries(data_dir: Path, min_confirmed: int = 5) -> list[Path]:
    """Find all video databases with confirmed boundaries.
    ...
    # Sort by number of confirmed boundaries (descending)
    video_dbs.sort(key=lambda x: x[1], reverse=True)

    print(f"\nFound {len(video_dbs)} videos with >={min_confirmed} confirmed boundaries:")
...
def label_frame_pair(frame1: int, frame2: int, captions: list[tuple[int, int]]) -> str | None:
    ...
    Args:
        ...
        captions: List of (start, end) caption boundaries
    ...
    if caption1 and caption2:
        if caption1 == caption2:
            ...
            return "different"  # Different captions (boundary transition)
...
def main():
    ...
    default_training_db = script_dir.parent.parent / "local" / "caption_boundaries_training.db"
    ...
    parser.add_argument(
        "--name",
        type=str,
        default=f"confirmed_boundaries_{datetime.now().strftime('%Y%m%d')}",
        help="Dataset name",
    )
    parser.add_argument(
        "--description",
        type=str,
        default="Training dataset from confirmed caption boundaries",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Minimum confirmed boundaries per video (default: 5)",
    )
    parser.add_argument(
        "--max-videos",
        type=int,
    ...
    print("Caption Boundary Training Dataset Creator")
    print("=" * 60)

    # Find videos
    video_dbs = find_videos_with_confirmed_boundaries(args.data_dir, args.min_confirmed)

    if not video_dbs:
        print(f"\n❌ No videos found with >= {args.min_confirmed} confirmed boundaries")
        return 1
