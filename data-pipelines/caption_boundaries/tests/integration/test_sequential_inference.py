...
for boundary detection, including bidirectional inference.
"""
...
class TestSequentialFrameInference:
    ...
    def mock_predictor(self, tmp_path):
        ...
        with patch("caption_boundaries.inference.batch_predictor.create_model") as mock_create:
...
class TestBoundariesDatabaseCreation:
    """Tests for boundaries database creation from inference results."""

    @pytest.mark.integration
    def test_create_boundaries_db_from_pair_results(self, tmp_path):
        """Should create valid boundaries database from PairResults."""
    ...
    @pytest.mark.integration
    def test_boundaries_db_enforces_unique_pairs(self, tmp_path):
...
class TestQualityChecksIntegration:
    ...
    def test_quality_checks_on_valid_sequence(self, tmp_path):
        """Quality checks should pass for valid boundary sequence."""
        boundaries = [
    ...
    @pytest.mark.integration
    def test_quality_checks_detect_consecutive_boundaries(self, tmp_path):
        """Quality checks should flag consecutive boundary predictions."""
        boundaries = [
...
class TestEndToEndSequentialInference:
    ...
    @pytest.fixture
    def mock_predictor_with_boundary_detection(self, tmp_path):
        """Create predictor that detects boundaries at specific positions."""
        layout_db_path = create_mock_layout_db(tmp_path)
        checkpoint_path = create_mock_checkpoint(tmp_path)
        ...
        with patch("caption_boundaries.inference.batch_predictor.create_model") as mock_create:
            ...
            def mock_forward(ocr_viz, _frame1, _frame2, _spatial):
                ...
                for i in range(batch_size):
                    idx = call_count[0] + i
                    # Simulate boundary at positions 5 and 15
    ...
    @pytest.mark.integration
    def test_full_pipeline_with_quality_checks(self, mock_predictor_with_boundary_detection, tmp_path):
        ...
        all_predictions = mock_predictor_with_boundary_detection.predict_batch(bidirectional_pairs)
        ...
        assert quality_result["quality_stats"]["total_boundaries"] == 19
