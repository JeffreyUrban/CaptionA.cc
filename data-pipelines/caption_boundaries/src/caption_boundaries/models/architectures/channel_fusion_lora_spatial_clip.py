...
class ChannelFusionLoRAPredictor(nn.Module):
    ...
    def __init__(
        ...
        # CLIP encoder for reference image embedding
        self.clip = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
        # Freeze CLIP encoder (only use for feature extraction)
        for param in self.clip.parameters():
            param.requires_grad = False
        self.clip_feat_dim = self.clip.config.projection_dim  # 512 for base CLIP

        self.font_proj = nn.Sequential(
            nn.Linear(self.clip_feat_dim, 256),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(256, 128),
        )
    ...
    def forward(
        ...
        Args:
            ...
            reference_image: (batch, 3, H, W) - Reference frame for CLIP encoding
        ...
        reference_resized = F.interpolate(reference_image, size=(224, 224), mode="bilinear", align_corners=False)
        reference_embedding = self.clip.get_image_features(pixel_values=reference_resized)  # (batch, 512)
        ...
        font_proj = self.font_proj(reference_embedding)  # (batch, 128)
...
@register_model("channel_fusion_lora_spatial_clip")
def create_channel_fusion_lora_spatial_clip(
    num_classes: int = 5,
    pretrained: bool = True,
    lora_rank: int = 16,
    lora_alpha: float = 16.0,
