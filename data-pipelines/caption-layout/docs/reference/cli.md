...
```bash
caption-layout [OPTIONS] [INPUT_FILE]
```
...
```bash
caption-layout --quiet input.log
```
...
```bash
caption-layout --progress large-file.log
```
...
```bash
caption-layout --stats-format json input.log
```
...
caption-layout --explain input.log 2> explain.log

# Debug with quiet mode (only explanations, no stats)
caption-layout --explain --quiet input.log

# Validate TEMPLATE_PLACEHOLDER
caption-layout --explain --TEMPLATE_PLACEHOLDER input.log 2>&1 | grep EXPLAIN
```
...
```bash
# TEMPLATE_PLACEHOLDER
caption-layout TEMPLATE_PLACEHOLDER.log > output.log
```
