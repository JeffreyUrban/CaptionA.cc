...
This document outlines the deployment strategy for distributing crop_frames to users.

## Overview

crop_frames is distributed through multiple channels to reach different user communities:
...
2. **Create GitHub Release** (Recommended: Use GitHub UI)
   - Go to: https://github.com/yourusername/crop_frames/releases/new
...
# Test installation
pip install --index-url https://test.pypi.org/simple/ --no-deps crop_frames
...
Create separate tap repository: `homebrew-crop_frames`

**Repository**: `https://github.com/yourusername/homebrew-crop_frames`

### Formula Template

File: `Formula/crop_frames.rb`
...
class Uniqseq < Formula
  ...
  homepage "https://github.com/yourusername/crop_frames"
  url "https://files.pythonhosted.org/packages/.../crop_frames-0.1.1.tar.gz"
  ...
  test do
    # Basic smoke test
    output = shell_output("#{bin}/crop_frames --version")
    assert_match "crop_frames version", output

    # Functional test
    (testpath/"test.txt").write("line1\nline2\nline3\nline1\nline2\nline3\n")
    output = shell_output("#{bin}/crop_frames --window-size 3 --quiet #{testpath}/test.txt")
    assert_equal "line1\nline2\nline3\n", output
  end
end
```
...
```bash
brew tap yourusername/crop_frames
brew install crop_frames
```
...
2. Test locally: `brew install --build-from-source ./Formula/crop_frames.rb`
3. Commit and push formula update
4. Users update: `brew upgrade crop_frames`
...
2. **Homebrew Tap** (Current Priority)
   - [ ] Create `homebrew-crop_frames` repository
