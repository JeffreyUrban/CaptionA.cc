...
dvc add !__local/data/_has_been_deprecated__!/boundaries.db

# This creates boundaries.db.dvc (metadata file)
# Add to git, but NOT the actual .db file
git add !__local/data/_has_been_deprecated__!/boundaries.db.dvc !__local/data/_has_been_deprecated__!/.gitignore
...
def _get_dataset_dvc_hash(self) -> str:
    ...
    result = subprocess.run(
        ["dvc", "get-url", "--rev", "HEAD", "!__local/data/_has_been_deprecated__!/boundaries.db.dvc"],
        capture_output=True,
        text=True,
    )
...
# Try to commit in !__local/data/_has_been_deprecated__!/ (blocked by custom script)
touch !__local/data/_has_been_deprecated__!/test.db
git add !__local/data/_has_been_deprecated__!/test.db
git commit -m "Test commit"
# Prevent data/model commits (comprehensive)..............................Failed
# - hook id: prevent-data-commits
#
# ❌ BLOCKED: !__local/data/_has_been_deprecated__!/test.db
#    Reason: Files in !__local/data/_has_been_deprecated__!/ must be tracked with DVC
```
...
# Update DVC tracking
dvc add !__local/data/_has_been_deprecated__!/boundaries.db
git add !__local/data/_has_been_deprecated__!/boundaries.db.dvc
...
stages:
  build_dataset:
    cmd: python -m caption_boundaries.data.dataset_builder --output !__local/data/_has_been_deprecated__!/boundaries.db
    deps:
      - caption_boundaries/data/dataset_builder.py
      - caption_boundaries/database/schema.py
    outs:
      - !__local/data/_has_been_deprecated__!/boundaries.db:
          cache: true
...
git filter-branch --force --index-filter \
  "git rm --cached --ignore-unmatch !__local/data/_has_been_deprecated__!/boundaries.db" \
