# Caption Frame Extents Detection

Deep learning pipeline for detecting caption frame transitions by comparing consecutive cropped caption frames.
...
caption_frame_extents train my_dataset --name production --max-samples 20000 --epochs 50
...
```bash
caption_frame_extents infer path/to/video/captions.db \
...
```bash
caption_frame_extents analyze path/to/video/captions.db
```
...
```
local/models/caption_frame_extents/datasets/{dataset_name}.db
```
