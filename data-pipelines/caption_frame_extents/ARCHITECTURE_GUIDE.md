...
The caption frame extents pipeline uses a **registry pattern** for model architectures, enabling rapid experimentation while maintaining full reproducibility.
...
All models for caption frame extents detection must:
...
- **Training**: `caption_frame_extents train dataset --architecture triple_backbone_resnet50`
...
- **Training**: `caption_frame_extents train dataset --architecture shared_backbone_resnet50`
...
- **Training**: `caption_frame_extents train dataset --architecture channel_fusion_lora_resnet50`
...
from caption_frame_extents.models.registry import register_model
...
```python
from caption_frame_extents.models import my_architecture  # noqa: F401
```
...
```bash
# Training
caption_frame_extents train my_dataset \
    --name exp_custom \
    --architecture my_custom_architecture \
    --pretrained

# Inference (automatically uses correct architecture from checkpoint)
caption_frame_extents infer video.db --checkpoint checkpoints/best.pt
```
...
```python
predictor = CaptionFrameExtentsPredictor("checkpoints/best.pt")
