# Modal Setup Guide for Caption Frame Extents Inference

Complete guide to deploying the caption frame extents inference service on Modal.
...
modal volume create caption-frame-extents-models
```
...
python scripts/upload_model_to_modal.py \
  --checkpoint local/models/caption_frame_extents/fusion_lora_spatial_mrn0fkfd.pt \
...
wandb artifact get jeffreyurban-msai-deeplearning/caption-frame-extents-detection/model-mrn0fkfd:latest \
  --root local/models/caption_frame_extents/
...
**Verify upload:**
```bash
modal volume ls caption-frame-extents-models
```
...
✓ Created objects.
├── 🔨 Created mount /Users/.../caption_frame_extents/src/caption_frame_extents
├── 🔨 Created image caption-frame-extents-inference-image
└── 🔨 Created function run_caption_frame_extents_inference_batch
✓ App deployed! 🎉

View Deployment: https://modal.com/apps/your-workspace/caption-frame-extents-inference
```
...
# Test caption frame extents inference flow
uv run python << 'EOF'
from flows.caption_frame_extents_inference import caption_frame_extents_inference_flow
...
modal app logs caption-frame-extents-inference --function run_caption_frame_extents_inference_batch
```
...
modal app logs caption-frame-extents-inference --run-id <run-id>
```
...
3. Trigger caption frame extents inference (auto or manual)
