...
```
data-pipelines/caption_frame_extents/src/caption_frame_extents/inference/config.py
```
...
def check_rate_limit(video_id: str) -> bool:
    ...
    key = f"caption_frame_extents_inference:{video_id}"
...
def check_queue_depth() -> int:
    """Check Prefect queue depth for caption frame extents inference."""
    # Get pending jobs from Supabase
    supabase = get_supabase_client()
    response = supabase.table("caption_frame_extents_inference_jobs").select("count").eq("status", "queued").execute()
...
FROM caption_frame_extents_inference_runs
...
FROM caption_frame_extents_inference_jobs
...
def test_deduplication():
    ...
    # Run inference
    result1 = caption_frame_extents_inference_flow(video_id, version, model)
    assert result1["status"] == "success"

    # Try again - should skip
    result2 = caption_frame_extents_inference_flow(video_id, version, model)
    assert result2["status"] == "skipped"
    assert result2["reason"] == "run_already_exists"

def test_rate_limiting():
    """Ensure videos can't be processed too frequently."""
    caption_frame_extents_inference_flow(video_id, version, model)

    # Immediate retry should fail
    with pytest.raises(ValueError, match="processed too recently"):
        caption_frame_extents_inference_flow(video_id, version, model, skip_if_exists=False)
...
def test_cost_validation():
    ...
    with pytest.raises(ValueError, match="Job too expensive"):
        caption_frame_extents_inference_flow(
            video_id,
            version,
            model,
...
def test_concurrency_limit():
    ...
    jobs = [caption_frame_extents_inference_flow.submit(...) for _ in range(10)]
...
def run_caption_frame_extents_inference_batch(...):
    # Start heartbeat
...
1. **Log to Supabase** - Creates record in `caption_frame_extents_inference_rejections` table
...
```
═══════════════════════════════════════════════════════
   Caption Frame Extents Inference Rejections Monitor
═══════════════════════════════════════════════════════
...
CREATE TRIGGER rejection_alert
  AFTER INSERT ON caption_frame_extents_inference_rejections
  FOR EACH ROW
