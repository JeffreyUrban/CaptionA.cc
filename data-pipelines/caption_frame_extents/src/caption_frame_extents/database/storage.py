...
from caption_frame_extents.database.schema import Base
...
def get_default_dataset_dir() -> Path:
    ...
    if git_root:
        return git_root / "local" / "models" / "caption_frame_extents" / "datasets"
    else:
        # Fallback for environments without git (e.g., Modal containers)
        return Path("/tmp") / "caption_frame_extents" / "datasets"
...
def get_dataset_db_path(dataset_name: str) -> Path:
    ...
    Example:
        ...
        Path("local/models/caption_frame_extents/datasets/production_v1.db")
    """
