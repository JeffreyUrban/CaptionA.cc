"""Central training database schema for caption frame extents detection.
...
Database location: local/caption_frame_extents_training.db
"""
...
class TrainingDataset(Base):
    ...
    # Provenance: Cropping (video_hash -> crop_region_version)
    crop_region_versions: Mapped[dict[str, int]] = mapped_column(JSON, nullable=False)
    ...
    __table_args__ = (
        CheckConstraint(
            "split_strategy IN ('random', 'show_based')", name="check_split_strategy"
        ),
        CheckConstraint(
            "train_split_ratio > 0 AND train_split_ratio < 1",
            name="check_train_split_ratio",
        ),
    )
...
class TrainingSample(Base):
    ...
    dataset_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("training_datasets.id"), nullable=False
    )
    ...
    crop_region_version: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    __table_args__ = (
        CheckConstraint(
            "label IN ('same', 'different', 'empty_empty', 'empty_valid', 'valid_empty')",
            name="check_label",
        ),
...
class OCRVisualization(Base):
    ...
    video_hash: Mapped[str] = mapped_column(
        String(64), ForeignKey("video_registry.video_hash"), nullable=False
    )
    ...
    # Visualization data
    visualization_path: Mapped[str] = mapped_column(
        Text, nullable=False
    ...
    __table_args__ = (
        CheckConstraint(
            "variant IN ('boundaries', 'centers', 'both', '3d_channels')",
            name="check_variant",
        ),
        # One visualization per video per variant per OCR version
        Index(
            "idx_unique_ocr_viz", "video_hash", "variant", "ocr_version", unique=True
        ),
    )
...
class TrainingOCRVisualization(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "variant IN ('boundaries', 'centers', 'both', '3d_channels')",
            name="check_training_ocr_variant",
        ),
...
class Experiment(Base):
    ...
    use_font_embedding: Mapped[bool] = mapped_column(
    ...
    __table_args__ = (
        CheckConstraint(
            "transform_strategy IN ('crop', 'mirror_tile', 'adaptive')",
            name="check_transform_strategy",
        ),
        CheckConstraint(
