"""Command-line interface for caption frame extents detection pipeline."""
...
app = typer.Typer(
    name="caption_frame_extents",
    help="Caption frame extents detection via deep learning on frame pair transitions",
...
def get_project_root() -> Path:
    ...
    try:
        ...
        # Fallback: assume we're in data-pipelines/caption_frame_extents/src/caption_frame_extents
...
def train(
    ...
    checkpoint_dir: Path = typer.Option(
        None,
        "--checkpoint-dir",
        help="Directory to save checkpoints (default: {project_root}/local/models/caption_frame_extents/experiments)",
    ),
    ...
    Examples:
        ...
        caption_frame_extents train my_dataset --name exp_baseline --max-samples 10000

        # Quick experiment (fast epochs)
        caption_frame_extents train my_dataset --name quick_test --max-samples 5000 --epochs 10

        # Ratio-based sampling (legacy)
        caption_frame_extents train my_dataset --name exp_ratio --sampling-ratio 3.0 --epochs 50

        # Disable balanced sampling (use all data)
        caption_frame_extents train my_dataset --name exp_full --no-balanced-sampling --epochs 50
    """
    from caption_frame_extents.data.transforms import ResizeStrategy
    from caption_frame_extents.database import get_dataset_db_path
    from caption_frame_extents.training import CaptionFrameExtentsTrainer
...
def version():
    ...
    from caption_frame_extents import __version__

    console.print(f"caption_frame_extents version: {__version__}")
