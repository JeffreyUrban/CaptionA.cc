"""Training loop for caption frame extents detection model.
...
from caption_frame_extents.database import Experiment, TrainingDataset, get_dataset_db
...
class BalancedBatchSampler(Sampler):
    ...
    Args:
        dataset: CaptionFrameExtentsDataset to sample from
...
class CaptionFrameExtentsTrainer:
    """Trainer for caption frame extents detection model.
    ...
    def _setup_datasets(self):
        ...
        # Train dataset
        self.train_dataset = CaptionFrameExtentsDataset(
        ...
        # Val dataset
        self.val_dataset = CaptionFrameExtentsDataset(
            dataset_db_path=self.dataset_db_path,
            split="val",
            transform_strategy=self.transform_strategy,
        )
        ...
        if self.balanced_sampling:
            ...
            self.train_loader = DataLoader(
                ...
                collate_fn=CaptionFrameExtentsDataset.collate_fn,
            ...
            for label in range(len(CaptionFrameExtentsDataset.LABELS)):
                label_name = CaptionFrameExtentsDataset.LABELS[label]
    ...
    def train_epoch(self, epoch: int) -> dict[str, float]:
        ...
        for idx, label_name in enumerate(CaptionFrameExtentsDataset.LABELS):
    ...
    def validate(self, epoch: int) -> dict[str, Any]:
        ...
        class_report = cast(
            ...
            classification_report(
                all_labels_np,
                all_preds_np,
                labels=list(range(len(CaptionFrameExtentsDataset.LABELS))),
                target_names=CaptionFrameExtentsDataset.LABELS,
                output_dict=True,
                zero_division="0",
            ),
        )
            ...
            wandb.log(
                {
                    "val/confusion_matrix_plot": wandb.plot.confusion_matrix(
                        ...
                        y_true=all_labels,
                        preds=all_preds,
                        class_names=CaptionFrameExtentsDataset.LABELS,
                    )
                },
                step=epoch,
            )
