# Caption Frame Extents Inference Service Design

## Overview

On-demand GPU inference service for caption frame extents detection. The service runs on Modal's serverless GPU infrastructure, processes frame pairs from VP9/WebM chunks, and stores results as immutable per-run SQLite databases in Wasabi.
...
├── captions.db                         # Caption Frame Extents annotations
├── caption_frame_extents/              # Inference results (per-run databases)
...
Each caption frame extents database is self-contained with metadata and results:
...
CREATE INDEX idx_inference_runs_video ON caption_frame_extents_inference_runs(video_id);
CREATE INDEX idx_inference_runs_version_model
  ON caption_frame_extents_inference_runs(video_id, cropped_frames_version, model_version);
...
CREATE TABLE captionacc_prod.caption_frame_extents_inference_jobs (
  ...
  inference_run_id UUID REFERENCES caption_frame_extents_inference_runs(id),
  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_inference_jobs_video ON caption_frame_extents_inference_jobs(video_id);
CREATE INDEX idx_inference_jobs_status ON caption_frame_extents_inference_jobs(status);
CREATE INDEX idx_inference_jobs_priority_status
  ON caption_frame_extents_inference_jobs(priority, status);
```
...
```python
from services.orchestrator.supabase_client import CaptionFrameExtentsInferenceRunRepository
...
def run_inference(
    ...
    db_path = create_caption_frame_extents_db(db_filename, results)

    # Upload to Wasabi
    wasabi_key = f"videos/{tenant_id}/{video_id}/caption_frame_extents/{db_filename}"
    upload_caption_frame_extents_db(db_path, wasabi_key)
...
db_path = download_caption_frame_extents_db(
    tenant_id=tenant_id,
    video_id=video_id,
...
```python
# Download two runs
run1_db = download_caption_frame_extents_db(tenant, video, "v1_model-abc123_run-x.db")
run2_db = download_caption_frame_extents_db(tenant, video, "v1_model-def456_run-y.db")
...
2. Flow triggers `caption_frame_extents_inference_flow` (via Prefect)
...
**Skip confirmed annotation interiors**: Only infer pairs at caption frame extents with non-confirmed neighbors. Could save 30-50% of inferences.
...
1. `data-pipelines/caption_frame_extents/src/caption_frame_extents/inference/service.py`
   - Modal inference functions (GPU workload)

2. `data-pipelines/caption_frame_extents/src/caption_frame_extents/inference/frame_extractor.py`
   - WebM chunk decoding (ported from web client)

3. `data-pipelines/caption_frame_extents/src/caption_frame_extents/inference/caption_frame_extents_db.py`
   - Caption Frame Extents DB creation, read/write operations, Wasabi upload/download

4. `data-pipelines/caption_frame_extents/src/caption_frame_extents/database/caption_frame_extents_schema.sql`
   - SQLite schema for caption frame extents DB (per-run)

5. `data-pipelines/caption_frame_extents/scripts/deploy_inference.py`
   - Deploy service to Modal

6. `supabase/migrations/20260107000000_caption_frame_extents_inference_tables.sql`
   - Schema for `caption_frame_extents_inference_runs` (completed runs registry)
   - Schema for `caption_frame_extents_inference_jobs` (active job queue)

7. `services/orchestrator/flows/caption_frame_extents_inference.py`
   - Prefect flow to trigger Modal function

### Modified Files

1. `services/orchestrator/supabase_client.py`
   - Add `CaptionFrameExtentsInferenceRunRepository` for run tracking
   - Add `CaptionFrameExtentsInferenceJobRepository` for job status tracking

2. `services/orchestrator/wasabi_client.py`
   - Add methods: `upload_caption_frame_extents_db()`, `download_caption_frame_extents_db()`
...
- Create SQLite schema for Caption Frame Extents DB
- Create Supabase migrations for run tracking
- Implement `caption_frame_extents_db.py` module
- Implement `CaptionFrameExtentsInferenceRunRepository`
- Test DB creation and Wasabi operations

### Phase 3: Full Service Implementation
- Implement complete Modal inference function
- Integrate with `CaptionFrameExtentsPredictor`
- Add error handling and retry logic
- Test on full video

### Phase 4: Prefect Integration
- Create caption frame extents inference Prefect flow
