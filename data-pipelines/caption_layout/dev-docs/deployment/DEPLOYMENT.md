...
This document outlines the deployment strategy for distributing caption-layout to users.

## Overview

caption-layout is distributed through multiple channels to reach different user communities:
...
# Test installation
pip install --index-url https://test.pypi.org/simple/ --no-deps caption_layout
...
Create separate tap repository: `homebrew-caption-layout`

**Repository**: `https://github.com/yourusername/homebrew-caption-layout`

### Formula Template

File: `Formula/caption-layout.rb`
...
class Uniqseq < Formula
  ...
  homepage "https://github.com/yourusername/caption-layout"
  url "https://files.pythonhosted.org/packages/.../caption-layout-0.1.1.tar.gz"
  ...
  test do
    # Basic smoke test
    output = shell_output("#{bin}/caption-layout --version")
    assert_match "caption-layout version", output

    # Functional test
    (testpath/"test.txt").write("line1\nline2\nline3\nline1\nline2\nline3\n")
    output = shell_output("#{bin}/caption-layout --window-size 3 --quiet #{testpath}/test.txt")
    assert_equal "line1\nline2\nline3\n", output
  end
end
```
...
```bash
brew tap yourusername/caption_layout
brew install caption_layout
```
...
2. Test locally: `brew install --build-from-source ./Formula/caption-layout.rb`
3. Commit and push formula update
4. Users update: `brew upgrade caption-layout`
...
2. **Homebrew Tap** (Current Priority)
   - [ ] Create `homebrew-caption-layout` repository
