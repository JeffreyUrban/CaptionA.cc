"""Database operations for caption_layout pipeline.
...
def get_database_path(output_dir: Path) -> Path:
    """Get annotations.db path from caption_layout output directory.

    Args:
        output_dir: Path to caption_layout output directory
                   (e.g., local/data/show_name/video_id/caption_layout)

    Returns:
        Path to annotations.db file

    Example:
        >>> get_database_path(Path("local/data/show_name/video_id/caption_layout"))
        Path("local/data/show_name/video_id/annotations.db")
    """
    # Go up one level from caption_layout to video directory
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # Delete frame after processing (except first frame for visualization)
