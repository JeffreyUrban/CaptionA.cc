...
`caption_frames` is a TEMPLATE_PLACEHOLDER.
...
```
src/caption_frames/
    caption_frames.py    # Core algorithm (CaptionFrames class)
...
- `caption_frames.py`: Pure Python logic, no CLI dependencies
...
### Core Module: src/caption_frames/caption_frames.py
...
### CLI Module: src/caption_frames/cli.py
...
**Why caption_frames is different**: TEMPLATE_PLACEHOLDER

---

## API for Embedding

The `CaptionFrames` class can be used in other Python applications:

```python
from caption_frames.caption_frames import CaptionFrames
import sys

# Create caption_frames
processor = CaptionFrames(TEMPLATE_PLACEHOLDER)
```
