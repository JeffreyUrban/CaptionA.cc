...
This document outlines the deployment strategy for distributing caption_frames to users.

## Overview

caption_frames is distributed through multiple channels to reach different user communities:
...
2. **Create GitHub Release** (Recommended: Use GitHub UI)
   - Go to: https://github.com/yourusername/caption_frames/releases/new
...
# Test installation
pip install --index-url https://test.pypi.org/simple/ --no-deps caption_frames

# Verify
caption_frames --version
caption_frames --help
```
...
Create separate tap repository: `homebrew-caption_frames`

**Repository**: `https://github.com/yourusername/homebrew-caption_frames`

### Formula Template

File: `Formula/caption_frames.rb`
...
class Uniqseq < Formula
  ...
  homepage "https://github.com/yourusername/caption_frames"
  url "https://files.pythonhosted.org/packages/.../caption_frames-0.1.1.tar.gz"
  ...
  test do
    # Basic smoke test
    output = shell_output("#{bin}/caption_frames --version")
    assert_match "caption_frames version", output

    # Functional test
    (testpath/"test.txt").write("line1\nline2\nline3\nline1\nline2\nline3\n")
    output = shell_output("#{bin}/caption_frames --window-size 3 --quiet #{testpath}/test.txt")
    assert_equal "line1\nline2\nline3\n", output
  end
end
```
...
```bash
brew tap yourusername/caption_frames
brew install caption_frames
```
...
2. Test locally: `brew install --build-from-source ./Formula/caption_frames.rb`
3. Commit and push formula update
4. Users update: `brew upgrade caption_frames`
...
2. **Homebrew Tap** (Current Priority)
   - [ ] Create `homebrew-caption_frames` repository
