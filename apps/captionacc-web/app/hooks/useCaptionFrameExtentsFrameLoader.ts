 ...
 *    - Loads caption frame extents + overview for long annotations (>500 frames)
 ...
 *
 * This is extracted from the main CaptionFrameExtentsWorkflow component to reduce complexity.
...
const CAPTION_FRAME_EXTENTS_BUFFER = 20 // Frames to pin around annotation caption frame extents (start-20 to end+20)
 ...
 * including a CAPTION_FRAME_EXTENTS_BUFFER frame buffer around caption frame extents for smooth navigation.
 */
function isChunkPinned(
  ...
  if (activeAnnotation) {
    const start = Math.max(0, activeAnnotation.start_frame_index - CAPTION_FRAME_EXTENTS_BUFFER)
    const end = activeAnnotation.end_frame_index + CAPTION_FRAME_EXTENTS_BUFFER
  ...
  if (nextAnnotation) {
    const start = Math.max(0, nextAnnotation.start_frame_index - CAPTION_FRAME_EXTENTS_BUFFER)
    const end = nextAnnotation.end_frame_index + CAPTION_FRAME_EXTENTS_BUFFER
    if (chunkStart <= end && chunkEnd >= start) {
      return true
    }
  }
 ...
 * a ±20 frame buffer around annotation caption frame extents for smooth navigation.
 */
function buildNextAnnotationQueue(
  ...
  // If annotation is too long, only preload caption frame extents + coarse overview
  if (span > MAX_PRELOAD_ANNOTATION_SPAN) {
    ...
    // range=128 ensures we get frames well beyond the ±20 caption frame extents buffer
    chunks.push(
      ...buildQueueForModulo(
  ...
  useEffect(() => {
    ...
    const loadFrameHierarchy = async () => {
      ...
      try {
        ...
        if (isJump && jumpTarget !== null && !isNaN(jumpTarget) && jumpTarget >= 0) {
          ...
          console.warn('[useCaptionFrameExtentsFrameLoader] Invalid jump target:', jumpTarget)
    ...
    const preloadNextAnnotation = async () => {
      ...
      if (jumpRequestedRef.current) {
        console.log('[useCaptionFrameExtentsFrameLoader] Deferring preload - jump in progress')
        return
      }
      ...
      try {
        ...
        console.log(
          `[useCaptionFrameExtentsFrameLoader] Preloading next annotation ${nextAnnotation.id} (${queue.length} chunks)`
        )
        ...
        while (queue.length > 0) {
          ...
          if (jumpRequestedRef.current) {
            console.log('[useCaptionFrameExtentsFrameLoader] Pausing preload - jump requested')
            isPreloading = false
            return
          }
        ...
        console.log(
          `[useCaptionFrameExtentsFrameLoader] Preloading complete for annotation ${nextAnnotation.id}`
        )
      } catch (error: unknown) {
