/**
 * Run migration 005 (add text_preferences) on all annotations.db files
 */
...
import { migrateTextPreferences } from '../app/db/migrate'
...
async function main() {
  ...
  for (const dbPath of dbPaths) {
    try {
      const wasMigrated = migrateTextPreferences(dbPath)
      if (wasMigrated) {
        migrated++
      } else {
        skipped++
