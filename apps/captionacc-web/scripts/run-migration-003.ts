/**
 * Run migration 003 (drop cropped_frame_ocr) on all annotations.db files
 */
...
import { migrateDropCroppedFrameOCR } from '../app/db/migrate'
...
async function main() {
  ...
  for (const dbPath of dbPaths) {
    try {
      const wasMigrated = migrateDropCroppedFrameOCR(dbPath)
      if (wasMigrated) {
        migrated++
      } else {
        skipped++
