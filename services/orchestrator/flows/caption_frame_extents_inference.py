"""Caption Frame Extents Inference Flow

Handles ML inference for caption frame extents detection:
...
from caption_frame_extents.inference.config import MODAL_CONFIG, format_frame_count_limit_message
from caption_frame_extents.inference.inference_repository import InferenceRunRow
...
@task(
    name="check-existing-caption-frame-extents-run",
    tags=["caption-frame-extents-inference", "deduplication"],
    log_prints=True,
)
def check_existing_run(
    ...
    """
    print("Checking for existing caption frame extents inference run:")
    ...
    # Query caption_frame_extents_inference_runs table
    response = (
        supabase.table("caption_frame_extents_inference_runs_runs")
        .select("*")
...
@task(
    ...
    tags=["caption-frame-extents-inference"],
    log_prints=True,
)
def generate_frame_pairs(
    ...
    """Generate consecutive frame pairs for caption frame extents detection.
    ...
    # See: data-pipelines/caption_frame_extents/src/caption_frame_extents/inference/config.py
...
@task(
    ...
    tags=["caption-frame-extents-inference", "modal", "gpu"],
    log_prints=True,
)
def invoke_modal_inference(
    ...
    try:
        ...
        from caption_frame_extents.inference.service import run_caption_frame_extents_inference_batch

        # Call Modal function (blocks until complete)
        result = run_caption_frame_extents_inference_batch.remote(  # type: ignore[attr-defined]
            video_id=video_id,
            tenant_id=tenant_id,
            cropped_frames_version=cropped_frames_version,
            model_version=model_version,
...
@flow(
    name="caption-frame-extents-inference",
    description="Run ML inference for caption frame extents detection",
    log_prints=True,
)
def caption_frame_extents_inference_flow(
    video_id: str,
    tenant_id: str,
    cropped_frames_version: int,
    model_version: str,
    ...
    """Run caption frame extents inference for a video.
    ...
    """
    print("\n" + "=" * 60)
    print("CAPTION FRAME EXTENTS INFERENCE FLOW")
    print("=" * 60)
    print(f"Video: {video_id}")
    ...
    from caption_frame_extents.inference.config import estimate_job_cost
    ...
    if cost_estimate["estimated_cost_usd"] > MODAL_CONFIG.max_cost_per_job_usd:
        rejection_message = (
            ...
            f"data-pipelines/caption_frame_extents/src/caption_frame_extents/inference/config.py"
        )
    ...
    create_table_artifact(
        key="caption-frame-extents-inference-summary",
        table={
            "Run ID": [run_id],
            "Video ID": [video_id],
        ...
        description=f"Caption Frame Extents inference completed for video {video_id}",
    )
...
def run_inference_for_video(
    ...
    # Run flow
    return caption_frame_extents_inference_flow(
        video_id=video_id,
        tenant_id=tenant_id,
        cropped_frames_version=cropped_frames_version,
        model_version=model_version,
