# Caption Frame Extents Marking Workflow - Detailed Design

# TODO: The database details in this doc are out of date.

**Status:** Design Document (Planning Phase)
**Parent:** [overview.md](overview.md)
**Purpose:** Visual identification of caption frame extents
...
│  │  ├────────────────────────────┤  │  ──────────────────  │  │
│  │  │ ►► Frame N (CURRENT) ◄◄    │  │  Caption Frame Extents:         │  │
...
2. Navigate to find caption frame extents
...
### Predicted Caption Frame Extents (Optional)
...
```typescript
interface CaptionFrameExtentsWorkflowState {
  videoId: string
...
```typescript
GET /api/annotations/next?mode=caption-frame-extents&video_id=video_001
  ...
  frames: [
    {
      ...
      ocr_text: "Hello, how are you?"  // Not used in caption frame extents workflow
    },
    ...
  ]
}
...
- **Pinned chunks** (never evicted):
  - Active annotation: `[start-20, end+20]` frames (smooth caption frame extents adjustments)
...
- Correct caption frame extents marking
...
3. Should predicted caption frame extents be auto-marked, or just suggested?
