...
CREATE INDEX idx_videos_layout_status ON captionacc_production.videos(layout_status) WHERE deleted_at IS NULL;
CREATE INDEX idx_videos_boundaries_status ON captionacc_production.videos(boundaries_status) WHERE deleted_at IS NULL;
CREATE INDEX idx_videos_text_status ON captionacc_production.videos(text_status) WHERE deleted_at IS NULL;

-- Add comment explaining the design
COMMENT ON COLUMN captionacc_production.videos.layout_status IS 'User action state: wait (processing), annotate (ready), done (approved), review (needs review), error';
COMMENT ON COLUMN captionacc_production.videos.boundaries_status IS 'User action state: wait (needs layout), annotate (ready), done (complete), review (needs review), error';
COMMENT ON COLUMN captionacc_production.videos.text_status IS 'User action state: wait (needs boundaries), annotate (ready), done (complete), review (needs review), error';
